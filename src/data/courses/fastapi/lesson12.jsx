// src/data/courses/html/lesson12.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson12/index.js';

const Lesson12Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson12/part1.jsx'
        const module = await import(`./lesson12/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 12 для этого урока
      const storageKeySuffix = `12_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 12
    const storageKeySuffix = `12_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson12Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson12Wrapper.title = "Дополнительные элементы формы";
// Lesson12Wrapper.duration = "25 мин";
// Lesson12Wrapper.difficulty = "Средний";

export default Lesson12Wrapper;