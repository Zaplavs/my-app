// src/data/courses/python/lesson2.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson2/index.js';

const Lesson2Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson2/part1.jsx'
        const module = await import(`./lesson2/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      const storageKeySuffix = `2_${currentSubLessonId}`; // '2' - это lessonId для lesson2
      const savedCode = localStorage.getItem(`lesson_${'python'}_${storageKeySuffix}_code`);
      if (savedCode !== null) {
        setUserCode(savedCode);
      } else {
        setUserCode('');
      }
      const savedCompleted = localStorage.getItem(`lesson_${'python'}_${storageKeySuffix}_completed`);
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    const storageKeySuffix = `2_${currentSubLessonId}`; // '2' - это lessonId
    setIsSaving(true);
    localStorage.setItem(`lesson_${'python'}_${storageKeySuffix}_code`, userCode);
  ...
  const handleCompleteForPart = () => {
    ...
    localStorage.setItem(`lesson_${'python'}_${storageKeySuffix}_completed`, newCompleted.toString());
  };
  ...
  const partTitle = currentSubLessonId ? `Часть: ${currentSubLessonId}` : 'Нет частей';
...
// Флаг для LessonPage
Lesson2Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson2Wrapper.title = "Переменные и типы данных"; // Можно добавить, если нужно
// Lesson2Wrapper.duration = "30 мин";
// Lesson2Wrapper.difficulty = "Начальный";

export default Lesson2Wrapper;