// src/data/courses/html/lesson22.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson22/index.js';

const Lesson22Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson22/part1.jsx'
        const module = await import(`./lesson22/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 22 для этого урока
      const storageKeySuffix = `22_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 22
    const storageKeySuffix = `22_${currentSubLessonId}`;
    setIsSaving(true);
