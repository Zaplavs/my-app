// src/data/courses/html/lesson20.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson20/index.js';

const Lesson20Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson20/part1.jsx'
        const module = await import(`./lesson20/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 20 для этого урока
      const storageKeySuffix = `20_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 20
    const storageKeySuffix = `20_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson20Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson20Wrapper.title = "Название урока 20";
// Lesson20Wrapper.duration = "XX мин";
// Lesson20Wrapper.difficulty = "Уровень сложности";

export default Lesson20Wrapper;