// src/data/courses/html/lesson11.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson11/index.js';

const Lesson11Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson11/part1.jsx'
        const module = await import(`./lesson11/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 11 для этого урока
      const storageKeySuffix = `11_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 11
    const storageKeySuffix = `11_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson11Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson11Wrapper.title = "Формы в HTML";
// Lesson11Wrapper.duration = "45 мин";
// Lesson11Wrapper.difficulty = "Средний";

export default Lesson11Wrapper;