// src/data/courses/html/lesson18.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson18/index.js';

const Lesson18Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson18/part1.jsx'
        const module = await import(`./lesson18/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 18 для этого урока
      const storageKeySuffix = `18_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 18
    const storageKeySuffix = `18_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson18Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson18Wrapper.title = "Название урока 18";
// Lesson18Wrapper.duration = "XX мин";
// Lesson18Wrapper.difficulty = "Уровень сложности";

export default Lesson18Wrapper;