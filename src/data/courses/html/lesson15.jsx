// src/data/courses/html/lesson15.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson15/index.js';

const Lesson15Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson15/part1.jsx'
        const module = await import(`./lesson15/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 15 для этого урока
      const storageKeySuffix = `15_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 15
    const storageKeySuffix = `15_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson15Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson15Wrapper.title = "Название урока 15";
// Lesson15Wrapper.duration = "XX мин";
// Lesson15Wrapper.difficulty = "Уровень сложности";

export default Lesson15Wrapper;