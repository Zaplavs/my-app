// src/data/courses/html/lesson6.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson6/index.js';

const Lesson6Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson6/part1.jsx'
        const module = await import(`./lesson6/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 6 для этого урока
      const storageKeySuffix = `6_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 6
    const storageKeySuffix = `6_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson6Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson6Wrapper.title = "Ссылки и изображения в HTML";
// Lesson6Wrapper.duration = "30 мин";
// Lesson6Wrapper.difficulty = "Начальный";

export default Lesson6Wrapper;