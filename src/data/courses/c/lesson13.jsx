// src/data/courses/html/lesson13.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson13/index.js';

const Lesson13Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson13/part1.jsx'
        const module = await import(`./lesson13/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 13 для этого урока
      const storageKeySuffix = `13_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 13
    const storageKeySuffix = `13_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson13Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson13Wrapper.title = "Название урока 13";
// Lesson13Wrapper.duration = "XX мин";
// Lesson13Wrapper.difficulty = "Уровень сложности";

export default Lesson13Wrapper;