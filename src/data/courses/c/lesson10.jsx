// src/data/courses/html/lesson10.jsx
import React, { useState, useEffect } from 'react';
import { ArrowLeft, ArrowRight, BookOpen, Code, CheckCircle, HelpCircle, Play, Save, Accessibility } from 'lucide-react';
// Импортируем список частей
import { subLessonIds as initialSubLessonIds } from './lesson10/index.js';

const Lesson10Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson10/part1.jsx'
        const module = await import(`./lesson10/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 10 для этого урока
      const storageKeySuffix = `10_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 10
    const storageKeySuffix = `10_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson10Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson10Wrapper.title = "Основы доступности (Accessibility) в HTML";
// Lesson10Wrapper.duration = "30 мин";
// Lesson10Wrapper.difficulty = "Средний";

export default Lesson10Wrapper;