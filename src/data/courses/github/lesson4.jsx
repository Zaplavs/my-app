// src/data/courses/html/lesson4.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson4/index.js';

const Lesson4Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson4/part1.jsx'
        const module = await import(`./lesson4/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 4 для этого урока
      const storageKeySuffix = `4_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 4
    const storageKeySuffix = `4_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson4Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson4Wrapper.title = "Заголовки и абзацы в HTML";
// Lesson4Wrapper.duration = "25 мин";
// Lesson4Wrapper.difficulty = "Начальный";

export default Lesson4Wrapper;