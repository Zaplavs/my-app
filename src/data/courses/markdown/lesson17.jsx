// src/data/courses/html/lesson17.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson17/index.js';

const Lesson17Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson17/part1.jsx'
        const module = await import(`./lesson17/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 17 для этого урока
      const storageKeySuffix = `17_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 17
    const storageKeySuffix = `17_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson17Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson17Wrapper.title = "Название урока 17";
// Lesson17Wrapper.duration = "XX мин";
// Lesson17Wrapper.difficulty = "Уровень сложности";

export default Lesson17Wrapper;