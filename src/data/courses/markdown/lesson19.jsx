// src/data/courses/html/lesson19.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson19/index.js';

const Lesson19Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson19/part1.jsx'
        const module = await import(`./lesson19/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 19 для этого урока
      const storageKeySuffix = `19_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 19
    const storageKeySuffix = `19_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson19Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson19Wrapper.title = "Название урока 19";
// Lesson19Wrapper.duration = "XX мин";
// Lesson19Wrapper.difficulty = "Уровень сложности";

export default Lesson19Wrapper;