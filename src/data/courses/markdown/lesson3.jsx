// src/data/courses/html/lesson3.jsx
...
const Lesson3Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 3 для этого урока
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 3
    const storageKeySuffix = `3_${currentSubLessonId}`;
    setIsSaving(true);
...
// Lesson3Wrapper.title = "Основные HTML-теги";
// Lesson3Wrapper.duration = "30 мин";
