// src/data/courses/html/lesson5.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson5/index.js';

const Lesson5Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson5/part1.jsx'
        const module = await import(`./lesson5/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 5 для этого урока
      const storageKeySuffix = `5_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 5
    const storageKeySuffix = `5_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson5Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson5Wrapper.title = "Списки в HTML";
// Lesson5Wrapper.duration = "25 мин";
// Lesson5Wrapper.difficulty = "Начальный";

export default Lesson5Wrapper;