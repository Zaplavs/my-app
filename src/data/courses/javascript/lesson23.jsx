// src/data/courses/javascript/lesson23.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson23/index.js';

const Lesson23Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson23/part1.jsx'
        const module = await import(`./lesson23/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'javascript' и lessonId = 23 для этого урока
      const storageKeySuffix = `23_${currentSubLessonId}`;
      const savedCode = localStorage.getItem(`lesson_${'javascript'}_${storageKeySuffix}_code`);
      if (savedCode !== null) {
        setUserCode(savedCode);
      } else {
        setUserCode('');
      }
      const savedCompleted = localStorage.getItem(`lesson_${'javascript'}_${storageKeySuffix}_completed`);
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'javascript' и lessonId = 23
    const storageKeySuffix = `23_${currentSubLessonId}`;
    setIsSaving(true);
    localStorage.setItem(`lesson_${'javascript'}_${storageKeySuffix}_code`, userCode);
  ...
  const handleCompleteForPart = () => {
    ...
    localStorage.setItem(`lesson_${'javascript'}_${storageKeySuffix}_completed`, newCompleted.toString());
  };
...
// Флаг для LessonPage
Lesson23Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson23Wrapper.title = "Название урока 23";
// Lesson23Wrapper.duration = "XX мин";
// Lesson23Wrapper.difficulty = "Уровень сложности";

export default Lesson23Wrapper;