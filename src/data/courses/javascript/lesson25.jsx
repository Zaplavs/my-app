// src/data/courses/html/lesson25.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson25/index.js';

const Lesson25Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson25/part1.jsx'
        const module = await import(`./lesson25/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 25 для этого урока
      const storageKeySuffix = `25_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 25
    const storageKeySuffix = `25_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson25Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson25Wrapper.title = "Название урока 25";
// Lesson25Wrapper.duration = "XX мин";
// Lesson25Wrapper.difficulty = "Уровень сложности";

export default Lesson25Wrapper;