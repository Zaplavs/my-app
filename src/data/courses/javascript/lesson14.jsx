// src/data/courses/html/lesson14.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson14/index.js';

const Lesson14Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson14/part1.jsx'
        const module = await import(`./lesson14/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 14 для этого урока
      const storageKeySuffix = `14_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 14
    const storageKeySuffix = `14_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson14Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson14Wrapper.title = "Название урока 14";
// Lesson14Wrapper.duration = "XX мин";
// Lesson14Wrapper.difficulty = "Уровень сложности";

export default Lesson14Wrapper;