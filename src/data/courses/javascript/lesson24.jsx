// src/data/courses/html/lesson24.jsx
...
import { subLessonIds as initialSubLessonIds } from './lesson24/index.js';

const Lesson24Wrapper = () => {
  ...
  useEffect(() => {
    ...
    const loadCurrentSubLesson = async () => {
      ...
      try {
        // Путь относительно этого файла: './lesson24/part1.jsx'
        const module = await import(`./lesson24/${currentSubLessonId}.jsx`);
        if (isMounted) {
    ...
    const loadStateForCurrentPart = () => {
      ...
      const currentSubLessonId = subLessonIds[currentPartIndex];
      // Предполагаем slug = 'html' и lessonId = 24 для этого урока
      const storageKeySuffix = `24_${currentSubLessonId}`;
  ...
  const handleSaveCodeForPart = () => {
     if (subLessonIds.length === 0 || currentPartIndex < 0 || currentPartIndex >= subLessonIds.length) return;
    const currentSubLessonId = subLessonIds[currentPartIndex];
    // Предполагаем slug = 'html' и lessonId = 24
    const storageKeySuffix = `24_${currentSubLessonId}`;
    setIsSaving(true);
...
// Флаг для LessonPage
Lesson24Wrapper.hasSubLessons = true;
// Эти свойства могут использоваться LessonPage, если он будет их поддерживать
// Lesson24Wrapper.title = "Название урока 24";
// Lesson24Wrapper.duration = "XX мин";
// Lesson24Wrapper.difficulty = "Уровень сложности";

export default Lesson24Wrapper;