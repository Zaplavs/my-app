// src/components/codemaze/CodeMazeLevel2.jsx
import React, { useState } from 'react';

export default function CodeMazeLevel2({ onBack }) {
  const [workspace, setWorkspace] = useState([]);
  const [result, setResult] = useState('');
  const [isCorrect, setIsCorrect] = useState(null);
  const [playerPosition, setPlayerPosition] = useState({ x: 0, y: 0 });
  const [exitPosition] = useState({ x: 0, y: 2 });
  ...
  const correctSolution = ['move_down()', 'move_down()'];
  ...
  const runCode = () => {
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Вы нашли путь к выходу с двумя поворотами!');
      setPlayerPosition({ x: 0, y: 2 });
    } else {
      setResult('Попробуйте еще раз. Нужно пройти путь с двумя поворотами');
    }
  };
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-blue-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Лабиринт кода - Уровень 2</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Два поворота</h2>
                <p className="text-gray-300">Пройдите путь с двумя поворотами к выходу</p>
              </div>
