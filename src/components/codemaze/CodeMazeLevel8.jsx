// src/components/codemaze/CodeMazeLevel8.jsx
import React, { useState } from 'react';

export default function CodeMazeLevel8({ onBack }) {
  const [workspace, setWorkspace] = useState([]);
  const [result, setResult] = useState('');
  const [isCorrect, setIsCorrect] = useState(null);
  const [playerPosition, setPlayerPosition] = useState({ x: 0, y: 0 });
  const [exitPosition] = useState({ x: 8, y: 9 });
  const [walls] = useState([
    ...
    { x: 0, y: 9 }, { x: 2, y: 9 }, { x: 4, y: 9 }, { x: 6, y: 9 }
  ]);
  ...
  const puzzlePieces = [
    ...
    { id: 6, code: 'elif path_is_blocked():', type: 'condition' },
    { id: 7, code: 'else:', type: 'condition' },
    { id: 8, code: 'while not_at_exit():', type: 'loop' },
    { id: 9, code: 'try:', type: 'exception' },
    { id: 10, code: 'except:', type: 'exception' },
    { id: 11, code: 'and', type: 'logic' },
    { id: 12, code: 'or', type: 'logic' }
  ];

  const correctSolution = [
    'while not_at_exit():',
    'if path_is_clear() and move_right():',
    'move_right()',
    'elif path_is_clear() and move_down():',
    'move_down()',
    'elif path_is_clear() and move_left():',
    'move_left()',
    'else:',
    'move_up()'
  ];
  ...
  const runCode = () => {
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Вы успешно прошли сложный лабиринт с множеством условий!');
      setPlayerPosition({ x: 8, y: 9 });
    } else {
      setResult('Попробуйте еще раз. Нужно использовать сложные условия и логику');
    }
  };
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-blue-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Лабиринт кода - Уровень 8</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Мастер лабиринтов</h2>
                <p className="text-gray-300">Сложный лабиринт с множеством условий и логики</p>
              </div>
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-8">
                ...
                <div className="flex justify-center">
                  <div className="grid grid-cols-9 gap-0.5 bg-gray-900 p-4 rounded-lg">
                    {renderGameBoard()}
                  </div>
                </div>
