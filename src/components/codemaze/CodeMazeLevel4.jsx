// src/components/codemaze/CodeMazeLevel4.jsx
import React, { useState } from 'react';

export default function CodeMazeLevel4({ onBack }) {
  const [workspace, setWorkspace] = useState([]);
  const [result, setResult] = useState('');
  const [isCorrect, setIsCorrect] = useState(null);
  const [playerPosition, setPlayerPosition] = useState({ x: 0, y: 0 });
  const [exitPosition] = useState({ x: 4, y: 2 });
  ...
  // Несколько возможных правильных решений
  const correctSolutions = [
    ['move_right()', 'move_right()', 'move_right()', 'move_right()', 'move_down()', 'move_down()'],
    ['move_down()', 'move_down()', 'move_right()', 'move_right()', 'move_right()', 'move_right()'],
    ['move_right()', 'move_down()', 'move_right()', 'move_down()', 'move_right()', 'move_right()']
  ];
  ...
  const runCode = () => {
    ...
    // Проверяем, находится ли игрок рядом с выходом
    const isPlayerNearExit = (
      (Math.abs(playerPosition.x - exitPosition.x) <= 1 && playerPosition.y === exitPosition.y) ||
      (Math.abs(playerPosition.y - exitPosition.y) <= 1 && playerPosition.x === exitPosition.x) ||
      (playerPosition.x === exitPosition.x && playerPosition.y === exitPosition.y)
    );
    
    // Проверяем, есть ли правильное решение среди возможных
    const isSolutionCorrect = correctSolutions.some(solution => 
    ...
    if (isSolutionCorrect) {
      ...
      setPlayerPosition({ x: 4, y: 2 });
    } else {
      setResult('Попробуйте еще раз. Нужно найти путь к выходу');
    }
  };
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-blue-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Лабиринт кода - Уровень 4</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Сложный выбор</h2>
                <p className="text-gray-300">Найдите путь к выходу - есть несколько решений!</p>
              </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6">
              ...
              <p className="text-yellow-300 mb-4">Подсказка: Вы можете дойти до выхода разными путями!</p>
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-8">
                ...
                <div className="mt-4 text-center text-gray-300">
                  <p>🚶 - Игрок | 🚪 - Выход</p>
                  <p className="mt-2 text-yellow-400">Цель: Добраться до выхода любым путем!</p>
                </div>
              </div>
