// src/components/codemaze/CodeMazeLevel6.jsx
import React, { useState } from 'react';

export default function CodeMazeLevel6({ onBack }) {
  const [workspace, setWorkspace] = useState([]);
  const [result, setResult] = useState('');
  const [isCorrect, setIsCorrect] = useState(null);
  const [playerPosition, setPlayerPosition] = useState({ x: 0, y: 0 });
  const [exitPosition] = useState({ x: 6, y: 6 });
  ...
  const puzzlePieces = [
    ...
    { id: 5, code: 'while has_path():', type: 'loop' },
    { id: 6, code: 'for i in range(3):', type: 'loop' },
    { id: 7, code: 'if path_is_clear():', type: 'condition' },
    { id: 8, code: 'break', type: 'control' },
    { id: 9, code: 'continue', type: 'control' }
  ];
  ...
  const runCode = () => {
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Вы успешно использовали циклы для навигации!');
      setPlayerPosition({ x: 6, y: 6 });
    } else {
      setResult('Попробуйте еще раз. Нужно использовать циклы для прохождения');
    }
  };
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-blue-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Лабиринт кода - Уровень 6</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Циклический путь</h2>
                <p className="text-gray-300">Используйте циклы для навигации по лабиринту</p>
              </div>
