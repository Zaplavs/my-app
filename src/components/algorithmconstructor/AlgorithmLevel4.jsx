// src/components/algorithmconstructor/AlgorithmLevel4.jsx
import React, { useState } from 'react';

export default function AlgorithmLevel4({ onBack }) {
  ...
  const puzzlePieces = [
    ...
    { id: 2, code: 'print("Привет")', type: 'statement' },
    { id: 3, code: 'i = 0', type: 'assignment' },
    { id: 4, code: 'while i < 3:', type: 'loop' },
    { id: 5, code: 'i = i + 1', type: 'assignment' },
    { id: 6, code: 'print(i)', type: 'statement' }
  ];

  const correctSolution = ['for i in range(5):', 'print("Привет")'];
  ...
  const runCode = () => {
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Программа выведет: Привет (5 раз)');
    } else {
      setResult('Попробуйте еще раз. Нужно создать цикл, который выводит "Привет" 5 раз');
    }
  };
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-purple-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Алгоритмический Конструктор - Уровень 4</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Циклы</h2>
                <p className="text-gray-300">Создайте программу, которая выводит "Привет" 5 раз с помощью цикла</p>
              </div>
