// src/components/algorithmconstructor/AlgorithmLevel7.jsx
import React, { useState } from 'react';

export default function AlgorithmLevel7({ onBack }) {
  const [workspace, setWorkspace] = useState([]);
  const [result, setResult] = useState('');
  const [isCorrect, setIsCorrect] = useState(null);

  const puzzlePieces = [
    { id: 1, code: 'student = {"name": "Анна", "age": 20}', type: 'assignment' },
    { id: 2, code: 'print(student["name"])', type: 'statement' },
    { id: 3, code: 'student["grade"] = "A"', type: 'assignment' },
    { id: 4, code: 'print(student)', type: 'statement' },
    { id: 5, code: 'for key in student:', type: 'loop' },
    { id: 6, code: 'print(key, student[key])', type: 'statement' }
  ];

  const correctSolution = ['student = {"name": "Анна", "age": 20}', 'print(student["name"])'];
  ...
  const runCode = () => {
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Программа выведет: Анна');
    } else {
      setResult('Попробуйте еще раз. Нужно создать словарь студента и вывести его имя');
    }
  };
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-purple-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Алгоритмический Конструктор - Уровень 7</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Словари</h2>
                <p className="text-gray-300">Создайте словарь студента с именем и возрастом, затем выведите имя</p>
              </div>
