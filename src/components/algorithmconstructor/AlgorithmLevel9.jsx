// src/components/algorithmconstructor/AlgorithmLevel9.jsx
import React, { useState } from 'react';

export default function AlgorithmLevel9({ onBack }) {
  const [workspace, setWorkspace] = useState([]);
  const [result, setResult] = useState('');
  const [isCorrect, setIsCorrect] = useState(null);

  const puzzlePieces = [
    { id: 1, code: 'try:', type: 'exception' },
    { id: 2, code: 'number = int("abc")', type: 'assignment' },
    { id: 3, code: 'except ValueError:', type: 'exception' },
    { id: 4, code: 'print("Ошибка преобразования")', type: 'statement' },
    { id: 5, code: 'except ZeroDivisionError:', type: 'exception' },
    { id: 6, code: 'print("Деление на ноль")', type: 'statement' },
    { id: 7, code: 'result = 10 / 0', type: 'assignment' },
    { id: 8, code: 'print("Программа завершена")', type: 'statement' }
  ];

  const correctSolution = ['try:', 'number = int("abc")', 'except ValueError:', 'print("Ошибка преобразования")'];
  ...
  const runCode = () => {
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Программа выведет: Ошибка преобразования');
    } else {
      setResult('Попробуйте еще раз. Нужно обработать ошибку преобразования строки в число');
    }
  };
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-purple-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Алгоритмический Конструктор - Уровень 9</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Исключения</h2>
                <p className="text-gray-300">Создайте программу, которая обрабатывает ошибку преобразования строки в число</p>
              </div>
