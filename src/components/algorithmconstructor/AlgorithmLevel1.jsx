// src/components/algorithmconstructor/AlgorithmLevel1.jsx
import React, { useState } from 'react';

export default function AlgorithmLevel1({ onBack }) {
  const [workspace, setWorkspace] = useState([]);
  const [result, setResult] = useState('');
  const [isCorrect, setIsCorrect] = useState(null);

  const puzzlePieces = [
    { id: 1, code: 'print("Привет, Мир!")', type: 'statement' },
    { id: 2, code: 'name = "Алиса"', type: 'assignment' },
    { id: 3, code: 'print(name)', type: 'statement' },
    { id: 4, code: '# Это комментарий', type: 'comment' }
  ];

  const correctSolution = ['print("Привет, Мир!")'];
  ...
  const runCode = () => {
    // В реальной игре здесь был бы запуск кода
    // Для демонстрации просто проверим правильность
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Программа выведет: Привет, Мир!');
    } else {
      setResult('Попробуйте еще раз. Нужно вывести текст "Привет, Мир!"');
    }
  };
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-purple-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Алгоритмический Конструктор - Уровень 1</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Привет, Мир!</h2>
                <p className="text-gray-300">Соберите программу, которая выводит текст "Привет, Мир!"</p>
              </div>
