// src/components/algorithmconstructor/AlgorithmLevel6.jsx
import React, { useState } from 'react';

export default function AlgorithmLevel6({ onBack }) {
  const [workspace, setWorkspace] = useState([]);
  const [result, setResult] = useState('');
  const [isCorrect, setIsCorrect] = useState(null);

  const puzzlePieces = [
    { id: 1, code: 'numbers = [1, 2, 3, 4, 5]', type: 'assignment' },
    { id: 2, code: 'print(numbers[0])', type: 'statement' },
    { id: 3, code: 'print(len(numbers))', type: 'statement' },
    { id: 4, code: 'numbers.append(6)', type: 'method' },
    { id: 5, code: 'for num in numbers:', type: 'loop' },
    { id: 6, code: 'print(num)', type: 'statement' }
  ];

  const correctSolution = ['numbers = [1, 2, 3, 4, 5]', 'for num in numbers:', 'print(num)'];
  ...
  const runCode = () => {
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Программа выведет: 1, 2, 3, 4, 5 (по одному числу в строке)');
    } else {
      setResult('Попробуйте еще раз. Нужно создать список чисел и вывести все элементы');
    }
  };
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-purple-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Алгоритмический Конструктор - Уровень 6</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Списки</h2>
                <p className="text-gray-300">Создайте список чисел и выведите все его элементы с помощью цикла</p>
              </div>
