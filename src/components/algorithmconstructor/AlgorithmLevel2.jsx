// src/components/algorithmconstructor/AlgorithmLevel2.jsx
import React, { useState } from 'react';

export default function AlgorithmLevel2({ onBack }) {
  const [workspace, setWorkspace] = useState([]);
  const [result, setResult] = useState('');
  const [isCorrect, setIsCorrect] = useState(null);

  const puzzlePieces = [
    { id: 1, code: 'age = 25', type: 'assignment' },
    { id: 2, code: 'name = "Боб"', type: 'assignment' },
    { id: 3, code: 'print(name)', type: 'statement' },
    { id: 4, code: 'print(age)', type: 'statement' },
    { id: 5, code: 'city = "Москва"', type: 'assignment' },
    { id: 6, code: 'print(city)', type: 'statement' }
  ];

  const correctSolution = ['name = "Боб"', 'age = 25', 'print(name)', 'print(age)'];
  ...
  const runCode = () => {
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Программа выведет: Боб, 25');
    } else {
      setResult('Попробуйте еще раз. Нужно создать переменные name и age, затем вывести их значения');
    }
  };
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-purple-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Алгоритмический Конструктор - Уровень 2</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Переменные</h2>
                <p className="text-gray-300">Создайте переменные name и age, затем выведите их значения</p>
              </div>
