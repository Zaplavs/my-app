// src/components/algorithmconstructor/AlgorithmLevel5.jsx
import React, { useState } from 'react';

export default function AlgorithmLevel5({ onBack }) {
  const [workspace, setWorkspace] = useState([]);
  const [result, setResult] = useState('');
  const [isCorrect, setIsCorrect] = useState(null);

  const puzzlePieces = [
    { id: 1, code: 'def greet(name):', type: 'function' },
    { id: 2, code: 'print("Привет, " + name)', type: 'statement' },
    { id: 3, code: 'greet("Анна")', type: 'call' },
    { id: 4, code: 'return name', type: 'return' },
    { id: 5, code: 'greet("Петр")', type: 'call' },
    { id: 6, code: 'print(greet("Мария"))', type: 'call' }
  ];

  const correctSolution = ['def greet(name):', 'print("Привет, " + name)', 'greet("Анна")'];
  ...
  const runCode = () => {
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Программа выведет: Привет, Анна');
    } else {
      setResult('Попробуйте еще раз. Нужно создать функцию приветствия и вызвать её');
    }
  };
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-purple-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Алгоритмический Конструктор - Уровень 5</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Функции</h2>
                <p className="text-gray-300">Создайте функцию приветствия и вызовите её с параметром "Анна"</p>
              </div>
