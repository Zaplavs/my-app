// src/components/codesnake/CodeSnakeLevel5.jsx
import React, { useState } from 'react';

export default function CodeSnakeLevel5({ onBack }) {
  const [workspace, setWorkspace] = useState([]);
  const [result, setResult] = useState('');
  const [isCorrect, setIsCorrect] = useState(null);
  const [snakePosition, setSnakePosition] = useState({ x: 2, y: 0 });
  ...
  const [berryPositions] = useState([
    ...
    { x: 2, y: 2, collected: false } // Это стена, но ягода должна быть собрана другим путем
  ]);
  ...
  const puzzlePieces = [
    ...
    { id: 6, code: 'if can_move_right():', type: 'condition' },
    { id: 7, code: 'if can_move_left():', type: 'condition' }
  ];

  const correctSolution = [
    'move_left()', 'collect_berry()',
    'move_right()', 'move_right()', 'collect_berry()',
    'move_down()', 'move_left()', 'move_left()', 'collect_berry()'
  ];
  ...
  const runCode = () => {
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Змейка собрала все ягоды в правильном порядке!');
      setSnakePosition({ x: 0, y: 2 });
      setBerriesCollected(3);
    } else {
      setResult('Попробуйте еще раз. Нужно собрать ягоды в определенном порядке, избегая стен');
    }
  };
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-green-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Змейка программиста - Уровень 5</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Сложный путь</h2>
                <p className="text-gray-300">Соберите ягоды в определенном порядке: левая верхняя, правая верхняя, центральная</p>
              </div>
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-8">
                ...
                <div className="mt-4 text-center text-gray-300">
                  ...
                  <p className="mt-2">Собрано ягод: {berriesCollected}/3</p>
                  <p className="mt-2 text-yellow-400">Порядок сбора: ← верхняя, → верхняя, центральная</p>
                </div>
              </div>
