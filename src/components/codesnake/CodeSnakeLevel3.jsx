// src/components/codesnake/CodeSnakeLevel3.jsx
import React, { useState } from 'react';

export default function CodeSnakeLevel3({ onBack }) {
  ...
  const correctSolution = [
    'move_right()', 'move_right()', 'move_right()', 'move_right()', 'collect_berry()',
    'move_down()', 'move_down()', 'collect_berry()',
    'move_left()', 'move_left()', 'move_left()', 'move_left()', 'collect_berry()'
  ];
  ...
  const runCode = () => {
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Змейка собрала все три ягоды!');
      setSnakePosition({ x: 0, y: 2 });
      setBerriesCollected(3);
    } else {
      setResult('Попробуйте еще раз. Нужно собрать все три ягоды в правильном порядке');
    }
  };
        ...
        board.push(
          <div key={`${x}-${y}`} className={cellClass}>
            ...
            {berryPositions.map((berry, index) => (
              berry.x === x && berry.y === y && !berry.collected && (
                <span key={index}>🍓</span>
              )
            ))}
          </div>
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-green-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Змейка программиста - Уровень 3</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Три ягоды</h2>
                <p className="text-gray-300">Соберите три ягоды без столкновений: правая верхняя, правая нижняя, левая нижняя</p>
              </div>
