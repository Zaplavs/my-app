// src/components/codesnake/CodeSnakeLevel9.jsx
import React, { useState } from 'react';

export default function CodeSnakeLevel9({ onBack }) {
  const [workspace, setWorkspace] = useState([]);
  const [result, setResult] = useState('');
  const [isCorrect, setIsCorrect] = useState(null);
  const [snakePosition, setSnakePosition] = useState({ x: 3, y: 0 });
  ...
  const puzzlePieces = [
    ...
    { id: 8, code: 'if can_move_left():', type: 'condition' },
    { id: 9, code: 'if can_move_up():', type: 'condition' },
    { id: 10, code: 'if can_move_down():', type: 'condition' },
    { id: 11, code: 'continue', type: 'control' }
  ];

  const correctSolution = [
    'collect_berry()',
    'move_down()', 'collect_berry()', 'move_right()', 'collect_berry()',
    'move_down()', 'collect_berry()', 'move_left()', 'collect_berry()',
    'move_down()', 'collect_berry()', 'move_right()', 'collect_berry()',
    'move_down()', 'collect_berry()', 'move_left()', 'collect_berry()'
  ];
  ...
  const runCode = () => {
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Змейка собрала все ягоды в легендарном лабиринте!');
      setSnakePosition({ x: 1, y: 5 });
      setBerriesCollected(9);
    } else {
      setResult('Попробуйте еще раз. Нужно пройти самый сложный лабиринт и собрать все ягоды');
    }
  };
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-green-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Змейка программиста - Уровень 9</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Легендарный уровень</h2>
                <p className="text-gray-300">Профессиональная навигация змейки в самом сложном лабиринте</p>
              </div>
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-8">
                ...
                <div className="mt-4 text-center text-gray-300">
                  ...
                  <p className="mt-2">Собрано ягод: {berriesCollected}/9</p>
                </div>
              </div>
