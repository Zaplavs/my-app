// src/components/codesnake/CodeSnakeLevel2.jsx
import React, { useState } from 'react';

export default function CodeSnakeLevel2({ onBack }) {
  ...
  const correctSolution = ['move_right()', 'move_right()', 'collect_berry()', 'move_down()', 'move_down()', 'collect_berry()'];
  ...
  const runCode = () => {
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Змейка собрала обе ягоды!');
      setSnakePosition({ x: 2, y: 2 });
      setBerriesCollected(2);
    } else {
      setResult('Попробуйте еще раз. Нужно собрать обе ягоды по очереди');
    }
  };
      ...
      for (let x = 0; x < 3; x++) {
        ...
        board.push(
          <div key={`${x}-${y}`} className={cellClass}>
            ...
            {!berriesCollected && snakePosition.x !== x && snakePosition.y !== y && 
             berryPositions[0].x === x && berryPositions[0].y === y && !berryPositions[0].collected && '🍓'}
            {berriesCollected >= 1 && snakePosition.x !== x && snakePosition.y !== y && 
             berryPositions[1].x === x && berryPositions[1].y === y && !berryPositions[1].collected && '🍓'}
          </div>
        );
      }
    }
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-green-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Змейка программиста - Уровень 2</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Две ягоды</h2>
                <p className="text-gray-300">Соберите две ягоды по очереди: сначала первую, затем вторую</p>
              </div>
