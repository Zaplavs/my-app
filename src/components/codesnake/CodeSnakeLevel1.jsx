// src/components/codesnake/CodeSnakeLevel1.jsx
import React, { useState } from 'react';

export default function CodeSnakeLevel1({ onBack }) {
  const [workspace, setWorkspace] = useState([]);
  const [result, setResult] = useState('');
  const [isCorrect, setIsCorrect] = useState(null);
  const [snakePosition, setSnakePosition] = useState({ x: 0, y: 0 });
  const [berryPosition] = useState({ x: 2, y: 0 });
  ...
  const correctSolution = ['move_right()', 'move_right()', 'collect_berry()'];
  ...
  const runCode = () => {
    ...
    if (isSolutionCorrect) {
      setResult('Правильно! Змейка собрала ягоду!');
      setSnakePosition({ x: 2, y: 0 });
    } else {
      setResult('Попробуйте еще раз. Нужно добраться до ягоды и собрать её');
    }
  };
  ...
  const renderGameBoard = () => {
    ...
    for (let y = 0; y < 3; y++) {
      for (let x = 0; x < 5; x++) {
        ...
        } else if (berryPosition.x === x && berryPosition.y === y) {
        ...
        board.push(
          <div key={`${x}-${y}`} className={cellClass}>
            ...
            {berryPosition.x === x && berryPosition.y === y && '🍓'}
          </div>
        );
      }
    }
      ...
      <header className="bg-black/20 backdrop-blur-sm border-b border-green-500/20">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Змейка программиста - Уровень 1</h1>
            <button 
              onClick={onBack}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">Первые шаги</h2>
                <p className="text-gray-300">Соберите ягоду, двигаясь вправо</p>
              </div>
